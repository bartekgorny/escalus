...
-spec pre_init_per_suite(any(), any(), any()) -> tuple().
pre_init_per_suite(_, Config, State) ->
  {Config, State#state{suite_total = 0, tcs = []}}.

%% @doc Called after init_per_suite.
-spec post_init_per_suite(any(), any(), any(), any()) -> tuple().
post_init_per_suite(_Suite, _Config, Return, State) ->
  {Return, State}.

%% @doc Called before end_per_suite.
-spec pre_end_per_suite(any(), any(), any()) -> tuple().
pre_end_per_suite(_Suite, Config, State) ->
  {Config, State}.

%% @doc Called after end_per_suite.
-spec post_end_per_suite(any(), any(), any(), any()) -> tuple().
...
-spec pre_init_per_group(any(), any(), any()) -> tuple().
pre_init_per_group(Group, Config, State) ->
  print_group_enter(Group, State, "Starting"),
  {Config, State}.

%% @doc Called after each init_per_group.
-spec post_init_per_group(any(), any(), any(), any()) -> tuple().
post_init_per_group(_Group, _Config, Return, State) ->
  {Return, State}.

%% @doc Called after each end_per_group.
-spec pre_end_per_group(any(), any(), any()) -> tuple().
pre_end_per_group(_Group, Config, State) ->
  {Config, State}.

%% @doc Called after each end_per_group.
-spec post_end_per_group(any(), any(), any(), any()) -> tuple().
post_end_per_group(Group, _Config, Return, State) ->
  print_group_enter(Group, State, "Finished"),
  {Return, State}.

%% @doc Called before each test case.
-spec pre_init_per_testcase(any(), any(), any()) -> tuple().
pre_init_per_testcase(TC, Config, State) ->
  print_case_enter(TC, State, "Starting"),
  {Config, State#state{ts = now(), total =
  State#state.suite_total + 1}}.

%% @doc Called after each test case.
-spec post_end_per_testcase(any(), any(), any(), any()) -> tuple().
...
%% @doc Called after post_init_per_suite, post_end_per_suite,
%% post_init_per_group,
%% post_end_per_group and post_end_per_testcase if the suite,
%% group or test case failed.
-spec on_tc_fail(any(), any(), any()) -> any().
on_tc_fail(_TC, _Reason, State) ->
  State.

%% @doc Called when a test case is skipped by either user action
%% or due to an init function failing.
-spec on_tc_skip(any(), any(), any()) -> any().
